...
void JOY_init(ADC_HandleTypeDef *ADC_handle, TIM_HandleTypeDef *timer_handle)
{
	...
	// 2. Nastavitev začetnih vrednosti "joystick" sistemskih spremenljivk
		...
		// Smiselno nastavimo začetne ekstremne vrednosti pozicije "joysticka".
			...
			joystick.position_raw_min[X] = 931;
			joystick.position_raw_min[Y] = 723;
			...
			joystick.position_raw_max[X] = 3265;
			joystick.position_raw_max[Y] = 3216;
	...
	while (pressed_key != JOY_BTN_NONE)
	{
		...
		switch (pressed_key)
		{
		  case JOY_BTN_FIRE:
			  ...
			  LED_toggle(LED0);		// Spremenimo stanje LEDice 0.
...
{
	uint8_t relativePosition = JOY_get_axis_position(axis);

	uint8_t ledToLightUp = relativePosition / 12;
	printf("%d \n", ledToLightUp);
	switch(ledToLightUp){
		case 0: ledToLightUp = 0x01;
				break;
		case 1: ledToLightUp = 0x02;
						break;
		case 2: ledToLightUp = 0x04;
						break;
		case 3: ledToLightUp = 0x08;
						break;
		case 4: ledToLightUp = 0x10;
						break;
		case 5: ledToLightUp = 0x20;
						break;
		case 6: ledToLightUp = 0x40;
						break;
		case 7: ledToLightUp = 0x80;
						break;
		case 8: ledToLightUp = 0x80;
								break;
	}
	LEDs_write(ledToLightUp);
